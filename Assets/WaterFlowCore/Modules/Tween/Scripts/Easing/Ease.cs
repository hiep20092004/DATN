...
namespace WaterFlow.Core
{
    public class Ease
    {
        ...
        public static void Init(CustomEasingFunction[] easingFunctions)
        {
            ...
            if(!easingFunctions.IsNullOrEmpty())
            {
                for (int i = 0; i < customEasingFunctions.Length; i++)
                {
                    ...
                    customEasingFunctionsLink.TryAdd(functionNameHash, i);
                }
            }
        ...
        public static CustomEasingFunction GetCustomEasingFunction(string name)
        {
            ...
            if (customEasingFunctionsLink.TryGetValue(hash, out var easingIdx))
            {
                return customEasingFunctions[easingIdx];
            }

            Debug.LogError($"[Tween]: Custom easing {name} is missing!");
