...
namespace WaterFlow.Core
{
    public static class TransformTweenCases
    {
        ...
        /// <summary>
        /// Changes quaternion rotation of object with extra full turns before reaching the target.
        /// </summary>
        public static TweenCase DORotate(this Component tweenObject, Quaternion resultValue, float time, int extraTurns, float delay = 0, bool unscaledTime = false, UpdateMethod tweenType = UpdateMethod.Update)
        {
            return new RotateQuaternionWithTurns(tweenObject.transform, resultValue, extraTurns).SetDelay(delay).SetDuration(time).SetUnscaledMode(unscaledTime).SetUpdateMethod(tweenType).StartTween();
        }
        ...
        public class RotateQuaternionWithTurns : TweenCaseFunction<Transform, Quaternion>
        {
            private Vector3 rotationAxis;
            private float totalAngle;

            public RotateQuaternionWithTurns(Transform tweenObject, Quaternion resultValue, int extraTurns) : base(tweenObject, resultValue)
            {
                parentObject = tweenObject.gameObject;
                startValue = tweenObject.rotation;

                Quaternion delta = resultValue * Quaternion.Inverse(startValue);
                ...
                rotationAxis = axis;
                totalAngle = angle + extraTurns * 360f;
            }
            ...
            public override void Invoke(float deltaTime)
            {
                float currentAngle = totalAngle * Interpolate(state);
                tweenObject.rotation = Quaternion.AngleAxis(currentAngle, rotationAxis) * startValue;
            }
        }
