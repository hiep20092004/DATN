...
namespace WaterFlow.Core
{
    ...
    public class ProjectInitSettingsEditor : Editor
    {
        ...
        [MenuItem("Tools/Project Init Settings", priority = 50)]
        ...
        private void InitGenericMenu()
        {
            ...
            foreach (Type type in registeredTypes)
            {
                ...
                if(defineAttribute != null)
                {
                    ...
                    {
                        bool isAlreadyActive = initModules != null && initModules.Any(x => x && x.GetType() == type);
                        if (isAlreadyActive)
                        {
        ...
        private IEnumerable<Type> GetRegisteredAttributes()
        {
            ...
            {
                IEnumerable<Type> types = assembly.GetLoadableTypes().Where(type => type.IsClass && !type.IsAbstract && type.IsSubclassOf(typeof(InitModule)));
        ...
        private static RequiredModule[] GetRequiredModules(InitModule[] coreModules)
        {
            ...
            IEnumerable<Type> registeredTypes = AppDomain.CurrentDomain.GetAssemblies().SelectMany(s => s.GetLoadableTypes()).Where(type => type.IsClass && !type.IsAbstract && type.IsSubclassOf(typeof(InitModule)));
