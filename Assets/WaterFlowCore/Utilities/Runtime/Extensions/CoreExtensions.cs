...
namespace WaterFlow.Core
{
    public static class CoreExtensions
    {
        ...
        {
            if (count >= array.Length) return Array.Empty<T>();
        ...
        /// <summary>
        /// Check if a read-only collection (including IReadOnlyList) is null or empty.
        /// </summary>
            ...
            for (int i = 0; i < fieldInfos.Length; i++)
            {
                ...
                if (fieldType.IsClass && fieldType != typeof(string))
                {
                    if (fieldType.IsArray)
                    {
                        ...
                        if (elementType != null)
                        {
                            FieldInfo[] arrayFieldInfos = elementType.GetFields(BindingFlags.Instance |
        ...
        {
            return scrollRect.viewport.InverseTransformPoint(target.transform.position).y -
                scrollRect.viewport.rect.height / 2 > 0;
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void SnapTargetBottom(this ScrollRect scrollRect, RectTransform target, float offsetX = 0,
            float offsetY = 0)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void SnapTargetTop(this ScrollRect scrollRect, RectTransform target, float offsetX = 0,
            float offsetY = 0)
        {
            scrollRect.SnapToTarget(target.position, offsetX, offsetY);
        }

        public static void SnapToTarget(this ScrollRect scrollRect, Vector3 target, float offsetX = 0,
            float offsetY = 0)
        {
